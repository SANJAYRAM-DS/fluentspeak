...
async function renderDashboard() {
  ...
  if (title) title.textContent = `Good to see you, ${profile.full_name || me.email.split("@")[0]}.`;
  if (subtitle) subtitle.textContent = "Choose a topic, start a scenario, and keep your streak moving.";
...
async function renderChat() {
  ...
  if (currentLesson) currentLesson.textContent = selectedConversation.title || selectedConversation.topic?.title || "Practice session";

  thread.innerHTML = (conversationData.messages || [])
    .map(
      (message) => `
